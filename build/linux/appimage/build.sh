...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  # add update's url
  # CortexIDE - this line is important for AppImage updates
  sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|cortexide|binaries|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun

  # remove check so build in docker can succeed
  sed -i 's/grep docker/# grep docker/' pkg2appimage.AppDir/usr/share/pkg2appimage/functions.sh

  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|CortexIDE-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|cortexide-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|cortexide-insiders|g' recipe.yml
  else
    # CortexIDE branding
    sed -i 's|@@NAME@@|CortexIDE|g' recipe.yml
    sed -i 's|@@APPNAME@@|cortexide|g' recipe.yml
    sed -i 's|@@ICON@@|cortexide|g' recipe.yml
  fi
  ...
  rm -rf CortexIDE* # Clean up AppImage build artifacts
fi
