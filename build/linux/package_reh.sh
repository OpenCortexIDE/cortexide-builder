...
${indent}}`;
    ...
    if (getVSCodeSysrootStart >= 0) {
      ...
      for (let i = getVSCodeSysrootStart; i < Math.min(getVSCodeSysrootStart + 150, lines.length); i++) {
        if (lines[i].includes('fetchUrl') && (lines[i].includes('await') || lines[i].includes('const url') || lines[i].includes('let url'))) {
          ...
          if (contextLines.includes('VSCODE_SYSROOT_PREFIX') || contextLines.includes('gcc') || contextLines.includes('remote') || process.env.VSCODE_SYSROOT_PREFIX) {
...
${callIndent}try {
${callIndent}  ${lines.slice(i, fetchUrlEnd + 1).map(l => l.trim()).join(' ')}
${callIndent}  // Check for null return (remote sysroot not available)
${callIndent}  if (url === null || url === undefined) {
...
${callIndent}  if (err && err.message && err.message.includes('Could not find asset') && process.env.VSCODE_SYSROOT_PREFIX && process.env.VSCODE_SYSROOT_PREFIX.includes('-gcc-')) {
